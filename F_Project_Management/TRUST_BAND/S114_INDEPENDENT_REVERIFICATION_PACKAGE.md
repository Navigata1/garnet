...
A design for an external lane to *independently re-verify* the S114 kernel red-team finding on Garnet **v0.8.1** (research-grade prototype; NOT production / NOT v1.0). This document specifies **what to probe, what to run, and how it would be graded** if and when an independent lane executes it. It is calibrated to a single hard rule: the design lane (this document) **may not run, grade, or predict** the re-verification. S114's status — recorded verbatim in `CURRENT_STATE.md:15-16` as "a self-found and self-fixed HIGH red-team finding — not independently verified," and referred to throughout this document by its established project shorthand **self-verified** — is **unchanged** by this document.
